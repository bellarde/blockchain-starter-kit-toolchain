...
describe('Starting tests for blockchain-bellarde-app', function() {
  ...
  it('website title should be blockchain-bellarde-app', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('blockchain-bellarde-app');
    })
  });

  it('network-name should be bellarde-network@0.0.2',() => {
    element(by.css('.network-name')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('bellarde-network@0.0.2.bna');
    });
  });

  it('navbar-brand should be blockchain-bellarde-app',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('blockchain-bellarde-app');
    });
  });
